...
describe('Room list test - enter room', () => {
  it('Successfully join public room', () => {;
    cy.get('button[name=public]').eq(0).click();

    cy.url().should('include', '/room/room2');
  });

  it('Successfully join private room', () => {;
    cy.get('button[name=private]').eq(0).click();
    cy.get('input[name=password]').eq(1).type('test');
    cy.get('button[name=enter]').eq(1).click();
  ...
  it('Displays errors on invalid password', () => {;
    ...
    cy.get('button[name=enter]').eq(1).should('be.disabled');
  });
  ...
  it('Redirect user to room list on access denied', () => {;
    ...
    cy.get('.v-alert').should('contain', 'Access denied');
